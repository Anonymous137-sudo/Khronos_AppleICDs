...
static bool
ao46_context_is_valid(AO46ContextRef ctx)
{
    return ctx && ctx->st && ctx->pipe;
}

static void
ao46_initialize_context_state(AO46ContextRef ctx)
{
    if (!ctx) {
        return;
    }
...
static struct AO46ShareGroupRec *
ao46_create_share_group(AO46PixelFormatRef pix)
{
    struct AO46ShareGroupRec *group = calloc(1, sizeof(*group));
    if (!group) {
        return NULL;
    }

    group->retain_count = 1;
    group->renderer_id = pix ? pix->renderer_id : 0x414F3436;
    group->profile = pix ? pix->profile : kCGLOGLPVersion_GL3_Core;
    return group;
}

static struct AO46ShareGroupRec *
ao46_retain_share_group(struct AO46ShareGroupRec *group)
{
    if (group) {
        group->retain_count++;
    }
...
static void
ao46_release_share_group(struct AO46ShareGroupRec *group)
{
    if (!group) {
        return;
    }
...
static GLint *
ao46_context_enable_slot(AO46ContextRef ctx, CGLContextEnable pname)
{
    if (!ctx) {
        return NULL;
    }
...
static void
ao46_copy_context_compat_state(AO46ContextRef src, AO46ContextRef dst)
{
    if (!src || !dst) {
        return;
    }
    ...
    memcpy(dst->swap_rectangle, src->swap_rectangle, sizeof(dst->swap_rectangle));
    dst->virtual_screen = src->virtual_screen;
    dst->fullscreen_display_mask = src->fullscreen_display_mask;
    dst->swap_interval = src->swap_interval;
    dst->surface_order = src->surface_order;
    dst->surface_opacity = src->surface_opacity;
    memcpy(dst->surface_backing_size,
           src->surface_backing_size,
           sizeof(dst->surface_backing_size));
    dst->surface_volatile = src->surface_volatile;
    dst->swaps_in_flight = src->swaps_in_flight;
    dst->abort_on_gpu_restart_denied = src->abort_on_gpu_restart_denied;
    dst->context_priority_request = src->context_priority_request;
...
CGLError AO46QueryRendererInfo(GLuint display_mask, AO46RendererInfoRef *out_rend, GLint *out_nrend)
{
    ...
    rend->max_samples = 4;
    rend->sample_modes = kCGLMultisampleBit;
    rend->sample_alpha = 1;
                           ...
                           AO46ContextRef *out_ctx)
{
    ...
    *out_ctx = NULL;
    err = AO46MesaCreateContext(pix, share, &ctx);
    if (err != kCGLNoError || !ctx) {
        return err != kCGLNoError ? err : kCGLBadAlloc;
    }

    ao46_initialize_context_state(ctx);
    ...
    if (share && share->share_group) {
        ctx->share_group = ao46_retain_share_group(share->share_group);
    } else {
        ctx->share_group = ao46_create_share_group(pix);
        if (!ctx->share_group) {
            pthread_mutex_destroy(&ctx->lock);
            ctx->lock_initialized = false;
            AO46MesaDestroyContext(ctx);
...
CGLError AO46CopyContext(AO46ContextRef src, AO46ContextRef dst, GLbitfield mask)
{
    ...
    ao46_copy_context_compat_state(src, dst);
    return kCGLNoError;
}
...
void AO46DestroyContext(AO46ContextRef ctx)
{
    ...
    if (ctx->retain_count <= 1) {
        ...
        ao46_release_share_group(ctx->share_group);
        ctx->share_group = NULL;
    }
...
CGLError AO46LockContext(AO46ContextRef ctx)
{
    ...
    return pthread_mutex_lock(&ctx->lock) == 0 ? kCGLNoError : kCGLBadState;
}
...
CGLError AO46UnlockContext(AO46ContextRef ctx)
{
    ...
    return pthread_mutex_unlock(&ctx->lock) == 0 ? kCGLNoError : kCGLBadState;
}
